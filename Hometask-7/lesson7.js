...
var SNAKE_SPEED = 600; // Интервал между перемещениями змейки
...
function init() {
    ...
    */
    ...
    // События кнопок Старт, Новая игра и Ваш счет
    document.getElementById('snake-start').addEventListener('click', startGame);
    document.getElementById('snake-renew').addEventListener('click', refreshGame);
    document.getElementById('score-field').addEventListener('click', showSnakeLength);
...
function move() {
    ...
    if (direction == 'x-') {
        ...
        if (coord_x == (FIELD_SIZE_X-1)) coord_x = -1;
        new_unit = document.getElementsByClassName('cell-' + (coord_y) + '-' + (coord_x + 1))[0];
    }
    else if (direction == 'y+') {
...
function showScore (score) {
    var scoreNumber = document.getElementById('score-field');
    scoreNumber.innerText = 'Ваши очки: ' + score;
}

// общая длина змейки по нажатию кнопки Ваш счет

function showSnakeLength () {
    alert('Длина змейки: ' + snake.length);
